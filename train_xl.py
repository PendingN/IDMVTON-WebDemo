...
def main():
    ...
    unet.base_model.model.encoder_hid_proj.requires_grad_(True)
    ...
    # Collect trainable parameters as a concrete list so it can be reused
    ...
    # Prepare only the top-level models. `image_proj_model` is part of `unet` (encoder_hid_proj),
    # so pass only `unet` here to avoid double-wrapping submodules in DDP/FSdp.
    unet,unet_encoder,image_encoder,optimizer,train_dataloader,test_dataloader = accelerator.prepare(
        unet, unet_encoder, image_encoder, optimizer, train_dataloader, test_dataloader
    )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet), accelerator.accumulate(image_proj_model):
                ...
                def compute_time_ids(original_size, crops_coords_top_left = (0,0)):
                    ...
                    # Use height,width as SDXL expects (height, width)
                ...
                add_time_ids = torch.cat(
                    [compute_time_ids((args.height, args.width)) for i in range(bsz)]
                )
                ...
                # Use encoder_hid_proj that is attached to the UNet (avoids double-wrapping)
                ip_tokens = unet.encoder_hid_proj(image_embeds)
                ...
                if accelerator.sync_gradients:
                    # Clip grads for concrete params list
                    accelerator.clip_grad_norm_(params_to_opt, 1.0)

                optimizer.step()
                optimizer.zero_grad()

            # Only advance global step and the progress bar on gradient sync (actual update step)
            if accelerator.sync_gradients:
                ...
                # Checkpoint every `checkpointing_epoch` update steps
                if global_step % args.checkpointing_epoch == 0:
                    if accelerator.is_main_process:
                        # Unwrap to CPU-backed models before saving to avoid DDP prefixes
                        unwrapped_unet = accelerator.unwrap_model(unet)
                        save_path = os.path.join(args.output_dir, f"checkpoint-{global_step}")
                        os.makedirs(save_path, exist_ok=True)

                        # Save LoRA adapters / PEFT state
                        unwrapped_unet.save_pretrained(os.path.join(save_path, "unet_lora"))

                        # Save conv_in and encoder_hid_proj state dicts from the unwrapped model
                        try:
                            torch.save(
                                unwrapped_unet.base_model.model.encoder_hid_proj.state_dict(),
                                os.path.join(save_path, "encoder_hid_proj.bin"),
                            )
                        except Exception:
                            # fallback to attribute path if different
                            torch.save(
                                unwrapped_unet.encoder_hid_proj.state_dict(),
                                os.path.join(save_path, "encoder_hid_proj.bin"),
                            )

                        torch.save(
                            unwrapped_unet.base_model.model.conv_in.state_dict(),
                            os.path.join(save_path, "conv_in.bin"),
                        )
